...
@section('title', 'Isi Profil Perusahaan')
@include('partials.header.page',['title'=>'Isi Profil Perusahaan','withBack'=>"yes"])
@section('content')
<div class="data-current-page d-none">{{route("manage.company.profile")}}</div>
<form action="{{route('manage.company.profile.field.store',$lowerTitle)}}" method="post" enctype="multipart/form-data" onsubmit="toggleLoadingAction()">
    ...
    <button class="my-primary-bg-color-outline btn d-block w-100 " type="button" onclick="addItemPreview()">Tambah Item</button>
