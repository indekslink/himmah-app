...
@section('title','Kategori di '.$store->nama)
        ...
        <div class="col-lg-8 col-md-10">
            ...
            <ul class="nav nav-pills my-3 justify-content-between nav-fill flex-nowrap" id="nav" role="tablist">
                <li class="nav-item" onclick="filterAction(`{{route('store.show',[$store->slug])}}`)" role="presentation">
                    <button class="nav-link " id="tab-produk-tab" data-bs-toggle="pill" data-bs-target="#tab-produk" type="button" role="tab" aria-controls="tab-produk" aria-selected="true">Produk</button>
                </li>


                <li class="nav-item" onclick="filterAction(`{{route('kategori_toko',[$store->slug])}}`)" role="presentation">
                    <button class="nav-link active" id="tab-kategori-tab" data-bs-toggle="pill" data-bs-target="#tab-kategori" type="button" role="tab" aria-controls="tab-kategori" aria-selected="false">Kategori</button>
                </li>
            </ul>
            <div class="tab-content px-2" id="navContent">
                <div class="tab-pane fade " id="tab-produk" role="tabpanel" aria-labelledby="tab-produk-tab">
                    <div class="my-1">
                ...
                <div class="tab-pane fade show active" id="tab-kategori" role="tabpanel" aria-labelledby="tab-kategori-tab">
                    <ul class="list-group list-group-flush">
                        @forelse($store->categories as $ctg)
                        <li onclick="filterAction(`{{route('detail_product_of_categories',[$store->slug,$ctg->slug])}}`)" class="list-group-item d-flex align-items-center cursor-pointer px-0 justify-content-between">
                            <div class="d-flex align-items-center">
                                <img src="{{avatar($ctg->gambar,'/images/store/kategori/')}}" class="me-3" style="width: 60px;height:60px;object-fit:contain;" alt="gambar-{{$loop->iteration}}">
                                <div>
                                    <div class="fw-bold">{{$ctg->nama}}</div>
                                    <div class="text-muted"><small>{{$ctg->products->count()}} Produk</small></div>
                                </div>
                            </div>
                        ...
                        <div class="text-center pt-4 fs-3 fw-bold">
                            Toko ini belum membuat Kategori
                        </div>
                        @endforelse
                        @if($kategori_yang_diikuti != null)
                        <hr>
                        <div class="text-center">&mdash; Kategori Toko lain yang sedang terpakai &mdash;</div>
                        @foreach($kategori_yang_diikuti as $ctg)

                        <li onclick="filterAction(`{{route('getAllProductOfCategories',[$ctg->slug])}}`)" class="list-group-item d-flex align-items-center cursor-pointer px-0 justify-content-between">
                            <div class="d-flex align-items-center">
                                <img src="{{avatar($ctg->gambar,'/images/store/kategori/')}}" class="me-3" style="width: 60px;height:60px;object-fit:contain;" alt="gambar-{{$loop->iteration}}">
                                <div>
                                    <div class="fw-bold">{{$ctg->nama}}</div>
